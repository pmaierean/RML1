 ...
 */
public class LinearMovementFormatTest {
    private static final Logger logger = LogManager.getLogger(LinearMovementFormatTest.class);
    private LinearMovementFormat format = new LinearMovementFormat();

    @Test
    public void testInvalidNull() {
        try {
            String s = format.generateCommand(null);
            assertEquals("Expected format", "M;", s);
        } catch (Exception e) {
    ...
    public void testOneArgument() {
        try {
            ...
            assertEquals("Expected format", "M 123.1,456.1;", s);
        } catch (Exception e) {
            fail("Unexpected result for empty arguments");
        }
    }
    ...
    public void testTwoArguments() {
        try {
            ...
            String s = format.generateCommand(arguments);
            assertEquals("Expected format", "M 123.1,456.1,789.1,123.1;", s);
        } catch (Exception e) {
            fail("Unexpected result for empty arguments");
        }
    }
    ...
    public void testIsParseableSimple() {
        try {
            List<CommandArgument> args = format.parseArguments("M;");
            assertNotNull(args);
    ...
    public void testParseArgumentsWithSpace() {
        try {
            List<CommandArgument> args = format.parseArguments("M 123.5,234.5;");
            assertNotNull(args);
    ...
    public void testParseArgumentsMultipleWithSpace() {
        try {
            List<CommandArgument> args = format.parseArguments("M 123.5,234.5,67.2,21.1;");
            assertNotNull(args);
