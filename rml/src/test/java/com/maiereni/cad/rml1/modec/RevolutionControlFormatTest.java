 ...
 */
public class RevolutionControlFormatTest {
    private static final Logger logger = LogManager.getLogger(RevolutionControlFormatTest.class);
    private RevolutionControlFormat format = new RevolutionControlFormat();

    @Test
    public void testFormatNull() {
        try {
            String cmd = format.generateCommand(null);
            assertEquals("Expected command", "!RC;", cmd);
        } catch (Exception e) {
            fail();
        }
    }
    ...
    public void testFormatValid() {
        try {
            ...
            String cmd = format.generateCommand(args);
            assertEquals("Expected value", "!RC 123;", cmd);
        } catch (Exception e) {
    ...
    public void testParseBlankCommand() {
        try {
            List<CommandArgument> arguments = format.parseArguments("!RC;");
    ...
    public void testParseLong() {
        try {
            List<CommandArgument> arguments = format.parseArguments("!RC123;");
            assertNotNull(arguments);
