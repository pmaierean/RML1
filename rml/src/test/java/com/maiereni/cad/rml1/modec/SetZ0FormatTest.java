 ...
 */
public class SetZ0FormatTest {
    private static final Logger logger = LogManager.getLogger(SetZ0FormatTest.class);
    private SetZ0Format format = new SetZ0Format();

    @Test
    public void testFormatNull() {
        try {
            String cmd = format.generateCommand(null);
            assertEquals("Expected command", "!ZO;", cmd);
        } catch (Exception e) {
            fail();
        }
    }
    ...
    public void testFormatValid() {
        try {
            ...
            String cmd = format.generateCommand(args);
            assertEquals("Expected value", "!ZO 123.1;", cmd);
        } catch (Exception e) {
    ...
    public void testParseBlankCommand() {
        try {
            List<CommandArgument> arguments = format.parseArguments("!ZO;");
    ...
    public void testParseFloat() {
        try {
            List<CommandArgument> arguments = format.parseArguments("!ZO123.1;");
            assertNotNull(arguments);
    ...
    public void testParseIntWithSpace() {
        try {
            List<CommandArgument> arguments = format.parseArguments("!ZO 123.1;");
            assertNotNull(arguments);
            assertTrue(arguments.size() == 1);
            assertTrue(arguments.get(0) instanceof FloatArgument);
            FloatArgument f = (FloatArgument) arguments.get(0);
