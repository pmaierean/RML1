 ...
 */
public class PenUpFormatTest {
    private static final Logger logger = LogManager.getLogger(PenUpFormatTest.class);
    private PenUpFormat format = new PenUpFormat();

    @Test
    public void testInvalidNull() {
        try {
            String s = format.generateCommand(null);
            assertEquals("Expected format", "PU;", s);
        } catch (Exception e) {
    ...
    public void testOneArgument() {
        try {
            ...
            assertEquals("Expected format", "PU 123.1,456.1;", s);
        } catch (Exception e) {
            fail("Unexpected result for empty arguments");
        }
    }
    ...
    public void testTwoArguments() {
        try {
            ...
            String s = format.generateCommand(arguments);
            assertEquals("Expected format", "PU 123.1,456.1,789.1,123.1;", s);
        } catch (Exception e) {
            fail("Unexpected result for empty arguments");
        }
    }
    ...
    public void testIsParseableSimple() {
        try {
            List<CommandArgument> args = format.parseArguments("PU;");
            assertNotNull(args);
    ...
    public void testParseArgumentsWithSpace() {
        try {
            List<CommandArgument> args = format.parseArguments("PU 123.5,234.5;");
            assertNotNull(args);
    ...
    public void testParseArgumentsMultipleWithSpace() {
        try {
            List<CommandArgument> args = format.parseArguments("PU 123.5,234.5,67.2,21.1;");
            assertNotNull(args);
